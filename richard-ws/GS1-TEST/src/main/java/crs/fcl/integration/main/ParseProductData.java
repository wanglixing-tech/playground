...
public class ParseProductData {
	 ...
	 */
	public ParseProductData(String sourseFileName) throws DocumentException {
	...
	private List<NameText> findNameText(Node node) {
		...
		for (Node ntn : ntNodeList) {
			...
			nt.setTextContent(ntn.selectSingleNode(".//*[name()='Text']").getText());
			nameTextList.add(nt);
		}
		return nameTextList;
	}
	...
	private List<LongTextItems> findLongTextItemsList(Node node) {
		...
		for (Node ltiNode : longTextItemsNodeList) {
			...
			lti.setTextContent(findLongTextItemsText(ltiNode));
